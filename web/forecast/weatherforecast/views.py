import copy
from django.http import HttpResponse
from django.shortcuts import render
from keras.models import load_model
import joblib
from django.template import loader
import numpy as np
import pandas as pd
from sklearn.model_selection import train_test_split
from sklearn.preprocessing import MinMaxScaler
from sklearn.metrics import r2_score
import joblib
import pickle
from json import dumps
# Create your views here.

def index(request):
    model_name = "Bi_LSTM.h5"
    target = "Max Temp"
    filename = 'datas/Data.csv'
    n_days_for_prediction = 7
    template = loader.get_template("weatherforecast/index.html")
    
    if 'submit-process' in request.POST:
        n_days_for_prediction = int(request.POST.get("n-predict", 0))
        target = request.POST.get("features", "Max Temp")
        model_name = request.POST.get("models", "Bi_LSTM.h5")
    print(model_name)
    if (model_name == "svr_model.pkl"):
        with open('models/svr_model.pkl', 'rb') as model_file:
            model = pickle.load(model_file)
    else:
        model = load_model("models/" + model_name)
    
    dateList = pd.date_range(
        start="2023-06-01", periods=n_days_for_prediction).strftime('%m/%d/%Y').tolist()
    data = forecast(filename, model, n_days_for_prediction, target, model_name)
    data = list(zip(data, dateList))

    predict = []
    for maxTemp, date in data:
        predict.append(
            {
                "maxTemp": round((maxTemp-32)*5/9, 4),
                "date": date
            }
        )

    context = {
        "predict": predict,
        "target": target,
        "data":  dumps(predict),
        "n_day": n_days_for_prediction,
    }

    return HttpResponse(template.render(context, request))


def forecast(filename, model, n_days_for_prediction, target, model_name):

    # Load dữ liệu từ file CSV
    data = pd.read_csv(filename)

    # Chọn window và thuộc tính
    window_size = 7
    features = ["Max Temp", "Avg Temp", "Min Temp",
                "Max Dew Point", "Avg Dew Point", "Min Dew Point"]

    # Chuẩn bị dữ liệu
    x = data[features].values
    y = data[target].values
    x_train, x_test, y_train, y_test = train_test_split(
        x, y, test_size=0.2, shuffle=False)
    # Chuẩn hóa dữ liệu
    scaler = MinMaxScaler()
    scaler = scaler.fit(x_train)
    x_test = scaler.transform(x_test)

    testX = []
    for i in range(window_size, len(x_test)+1):
        testX.append(x_test[i - window_size:i, :])
    testX = np.array(testX)
    if (model_name == "svr_model.pkl"):
        testX = testX.reshape(testX.shape[0], -1)

    window_for_predict = testX[-1:]
    predicted_future = []

    pred = model.predict(testX)
    
    if (model_name == "svr_model.pkl"):
        pred = pred.reshape(-1, 1)

    prediction_copies = np.repeat(pred, len(features), axis=-1)
    predicted = scaler.inverse_transform(prediction_copies)[:,features.index(target)]
    r2 = r2_score(y_test[6:], predicted)

    for i in range(0, n_days_for_prediction):
        value_predicted = model.predict(window_for_predict)
        # test_model = copy.copy(model)
        # if (check_train(test_model, window_for_predict, value_predicted, testX, y_test[6:],scaler,features,target,model_name) > r2):
        #     if (model_name == "svr_model.pkl"):
        #         model.fit(window_for_predict, value_predicted)
        #     else:
        #         model.fit(window_for_predict,value_predicted, epochs=5, batch_size=64, verbose=0, shuffle=False)
        #     if (model_name == "svr_model.pkl"):
        #         with open('models/svr_model.pkl', 'wb') as model_file:
        #             pickle.dump(model,model_file)
        #     else:
        #         model.save("models/"+model_name)
        
        if (model_name == "svr_model.pkl"):
            value_copy = np.repeat(value_predicted.reshape(-1, 1), len(features), axis=-1)
        else:
            value_copy = np.repeat(value_predicted, len(features), axis=-1)

        result_predicted = scaler.inverse_transform(value_copy)
        predicted_future = np.append(predicted_future,result_predicted[:,features.index(target)], axis=0)
        window_for_predict = np.delete(window_for_predict, 0, axis=1)
        if (model_name == "svr_model.pkl"):
            window_for_predict = np.reshape(np.append(window_for_predict, value_predicted), (1, testX.shape[1]))
        else:
            window_for_predict = np.append(window_for_predict,np.reshape(value_copy, (1, 1, len(features))), axis=1)
        
    return predicted_future


# def check_train(model, train_x, train_y, x_test, y_test, scaler, features, target, model_name):
#     if (model_name == "svr_model.pkl"):
#         model.fit(train_x, train_y)
#     else:
#         model.fit(train_x, train_y, epochs=5,batch_size=64, verbose=0, shuffle=False)
#     if (model_name != "svr_model.pkl"):
#         model.reset_states()
#     pred = model.predict(x_test)
#     if (model_name == "svr_model.pkl"):
#         pred = pred.reshape(-1, 1)
#     prediction_copies = np.repeat(pred, len(features), axis=-1)
#     predicted = scaler.inverse_transform(prediction_copies)[:, features.index(target)]
#     check = r2_score(y_test, predicted)
#     return check
